...
let optimize (sygus : t) : t =
  ...
  in Log.debug (lazy ("Topological sort of functions: " ^ (String.concat ~sep:"; " ts_funcs)))
   ...
   ; let sygus = { sygus with
                   synth_variables = List.dedup_and_sort ~compare:Poly.compare
                                       ...
                                       @ (List.filter (get_used_vars sygus.trans_func.name) ~f:(fun (v, _) -> not (String.is_suffix ~suffix:"!" v)))
