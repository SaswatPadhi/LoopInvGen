...
\begin{figure}[!t]
    ...
    \begin{algfunction}
      {\Record}
      {\SyGuSINVTriplet\colon \TSyGuS, V\colon \TString[\,], n\colon \TInt}
      {A collection of program states $\States\colon \TState[\,]$.}
        \State $\States \gets \{\}$
        \While{\TRUE}
            \LeftComment{\hspace*{1.125em} Start with an \emph{unseen} model of the precondition.}
            \State{$m \gets \Call{GetModel}{\Delta \wedge P(m) \wedge (\bigwedge_{s \, \in \, \States} m \neq s), \; V}$}
            \IfThenElse{$m = \NONE$}{\textbf{break}}{$\States \gets \States \cup \{m\}$}
            \While{$|\States| < n$}
                \LeftComment{\hspace*{2.5em} Make a transition, i.e. execute a single iteration of the loop.}
...
\begin{figure}[!t]
    ...
    \begin{algfunction}
      {\Infer}
      {\SyGuSINVTriplet\colon \TSyGuS, \mathcal{Z}\colon \TState[\,]}
      {A sufficient loop invariant $\Inv\colon \TPred{\TState}$.}
        ...
        \While{\TRUE}
            \State{$B \gets \{\}$}
            \While{\TRUE}
                \State{$\delta \gets \Call{\PIE}{\mathcal{Z}, B}$}
                \State{$c \gets \Call{\Checker}{\forall p,q\colon \Delta(p) \; \Rightarrow \; \Inv(p) \wedge T(p,q)\!\Rightarrow\!\Inv(q)}$}
                \IfThenElse{$c = \NONE$}{\textbf{break}}{$B \gets B \cup \{c\}$}
            \EndWhile
            \State{$\Inv \gets \Inv \wedge \delta$}
            ...
            \If{$c \neq \NONE$}
                \State{$\mathcal{S} \gets \Call{RecordStatesFrom}{C,\: {\color{gray} k}}$}
                \Return{$\Call{\Infer}{\SyGuSINVTriplet, \mathcal{Z} \cup \mathcal{S}}$}
            \ElsIf{$\delta = \TRUE$} \textbf{break}
            \EndIf
        \EndWhile
        \Return{\Inv}
    \end{algfunction}}
...
Sumit Gulwani and Zachary Kincaid for access to the \tool{Escher} program synthesis tool;
